...
class _AppointmentScreenState extends State<AppointmentScreen> {
  ...
  List<DateTime> fullDates = []; // สำหรับเก็บวันที่คิวเต็มแล้ว
  int availableSlots = 0; // สำหรับเก็บจำนวนคิวที่เหลือ

  @override
  void initState() {
    super.initState();
    initializeDateFormatting(); // เตรียมการแสดงวันที่ในรูปแบบภาษาไทย
    _fetchFullDates(); // ดึงวันที่คิวเต็ม
  }

  // ดึงวันที่คิวเต็มจาก backend
  Future<void> _fetchFullDates() async {
    try {
      final response = await http.get(Uri.parse('http://localhost:3000/appointments/full-dates/${widget.quizSet.name}'));

      if (response.statusCode == 200) {
        List<dynamic> dates = json.decode(response.body);
        setState(() {
          fullDates = dates.map((date) => DateTime.parse(date['appointment_date'])).toList();
        });
      } else {
        print('Error fetching full dates');
      }
    } catch (error) {
      print('Error fetching full dates: $error');
    }
  }

  // ดึงจำนวนคิวที่เหลือสำหรับวันที่เลือก
  Future<void> _fetchAvailableSlots(DateTime date) async {
    try {
      final formattedDate = "${date.year}-${date.month}-${date.day}";
      final response = await http.get(Uri.parse('http://localhost:3000/appointments/available-slots/${widget.quizSet.name}/$formattedDate'));
  ...
  void _submitAppointment() async {
    if (_selectedDay != null && availableSlots > 0) {
      try {
      ...
      QuickAlert.show(
        ...
        title: 'Warning',
        text: 'กรุณาเลือกวันที่และตรวจสอบจำนวนคิวที่เหลือก่อนทำการลงนัด',
      );
    }
  }
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            TableCalendar(
              ...
              onDaySelected: (selectedDay, focusedDay) {
                setState(() {
                  ...
                  _fetchAvailableSlots(_selectedDay!);  // ดึงจำนวนคิวที่เหลือ
                });
              },
            ...
            SizedBox(height: 8),
            if (availableSlots > 0)
              Text('คิวที่เหลือ: $availableSlots', style: TextStyle(fontSize: 18)),
            if (availableSlots == 0)
              Text('คิวเต็มแล้ว', style: TextStyle(fontSize: 18, color: Colors.red)),
            SizedBox(height: 8),
            ElevatedButton.icon(
